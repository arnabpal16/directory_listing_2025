...
export async function generateStaticParams() {
  const movies = getAllMovies().filter(m => m.type === 'show');
...
export default async function ShowDetailPage({ params }: Props) {
  ...
  if (!movie || movie.type !== 'show') {
    notFound();
  }

  const allMovies = getAllMovies();
  const relatedMovies = allMovies
    .filter(m => m.genres.some(g => movie.genres.includes(g)) && m.id !== movie.id && m.type === 'show')
    .slice(0, 6);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <dl className="space-y-4">
                  <div>
                    ...
                    <dd className="text-slate-900 dark:text-white capitalize">TV Series</dd>
                  </div>
                  {movie.releaseYear && (
                    <div>
        ...
        {relatedMovies.length > 0 && (
          <section className="py-12 border-t border-slate-200 dark:border-slate-800">
            <h2 className="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white mb-8">Related Shows</h2>
